...
engine = create_engine('mysql://root:@localhost/dqn_basicmat_'+stock_name.lower())  # enter your password and database names here
...
#background.to_csv('file/background.csv')
...
validat['spy'] = web.DataReader('^GSPC', 'yahoo', '2013-01-01', '2018-01-01')['Adj Close']
validat['daily_ret'] = validat['stock_price'].pct_change(1)
validat['bt_ret'] = validat['portfolio_value'].pct_change(1)
validat['bm_ret'] = validat['spy'].pct_change(1)
validat['Benchmark'] = validat['bm_ret'].cumsum() + 1
...
print(f'Sortino ratio: {sortino(bn, 0.011):.4f}')
print(f'Maximun drawdown: {max_dd(bn)[0] * -100:.2f} %')
...
kpi = pd.DataFrame()
kpi['name'] = ['backtest', 'benchmark', 'price']
kpi['cagr'] = [CAGR(result) * 100, CAGR(bn) * 100, CAGR(test_data) * 100]
kpi['volality'] = [volatility(result), volatility(bn), volatility(test_data)]
kpi['shape_ratio'] = [sharpe(result, 0.011), sharpe(bn, 0.011), sharpe(test_data, 0.011)]
kpi['sortino_ratio'] = [sortino(result, 0.011), sortino(bn, 0.011), sortino(test_data, 0.011)]
kpi['max_dd'] = [max_dd(result)[0] * -100, max_dd(bn)[0] * -100 , max_dd(test_data)[0] * -100]
kpi['calmar'] = [calmer(result), calmer(bn), calmer(test_data)]
...
performance['spy'] = bn['Benchmark']
performance['daily_ret'] = performance['stock_price'].pct_change(1)
performance['bt_ret'] = performance['portfolio_value'].pct_change(1)
performance['bm_ret'] = performance['spy'].pct_change(1)
performance['Benchmark'] = performance['bm_ret'].cumsum() + 1
...
inform['industry'] = ['basic materials']
...
for table in temp_dir:
    ...
    temp_dir[table].reset_index(inplace=True)
    temp_dir[table].index.name = 'num'
    temp_dir[table].columns = ['name', 'value']
    temp_dir[table].to_sql(table.replace(' ','_').lower(), con=engine, if_exists='replace')
    #temp_dir[table].to_csv('file/' + table.replace(' ','_') +'.csv')
