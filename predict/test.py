...
df['Adj Close'].iloc[-test_size:].plot(figsize=(13,4), marker='o')
plt.title('test set')
plt.grid(True)
...
class RecurrentNetwork:
    ...
    def norm_data(self):
        train_norm = self.scaler.transform(self.train_set.reshape(-1, 1))
        test_norm = self.scaler.transform(self.test_set.reshape(-1, 1))
    ...
    def testing(self):
        ...
        plt.grid(True)
        plt.autoscale(axis='x', tight=True)
        plt.plot(self.df['Adj Close'].iloc[-(self.future + 10):], label='Stock price', marker='o')
        plt.plot(df1['pred'], label='predicted', marker='o')
        plt.title(f'Prediction {ticker}')
        plt.xlabel('Date')
        plt.legend()
        plt.savefig(f'fig/{ticker}.png')
...
with open(f'result/{ticker}.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
    pickle.dump([model.amse(), np.mean((np.abs(prediction - true_value) / true_value) * 100)], f)
    
plot = pd.DataFrame()
plot['true_value'] = true_value.reshape(1, -1).tolist()[0]
plot['prediction'] = prediction
plot.to_csv(f'plot/{ticker}.csv')